...
## modsrc. 20260624: global -> dtype: th.float32
dtype = th.float32
...
class EDM:
    ...
    ## > modsrc. 20260416: revise sigma_hat -> explicit .item() to avoid CPU-GPU sync on tensor comparison
    def sigma_hat(self, sigma, num_steps):
...
class LightningEDM(pl.LightningModule):
    ...
    def __init__(
        ...
        ## > modsrc. 20260416: cache for latent shape inference — avoids dummy tensor allocation on every sample() call
    ...
    def step(self, batch, batch_idx):
        ...
        ## > modsrc. 20260416. enclose encoder with autograd off via torch.no_grad() to prevent graph construction
    ...
    def validation_step(self, batch, batch_idx):
        ...
        ## > modsrc. 20260415: turning off autograd -> prevent memleak
        with th.no_grad():
            loss = self.step(batch, batch_idx)
        self.log("validation/loss", loss.detach().item(), sync_dist=True)        
        ## src >
        # return loss
        ## modsrc > 20260422 - add .item()
        return loss.detach()
    ...
    def sample(self, shape, cond_sample=None, cond=None):
        ...
        ## > modsrc. 20260415: change dtype lessen memutil
        if self.autoencoder:
            if cond_sample is not None:
                cond_sample = self.autoencoder.encode(cond_sample)

            ## src: infer latent shape
            #dummy = th.zeros(shape, device=self.device)
            #latent = self.autoencoder.encode(dummy)
            #shape = latent.shape

            ## > modsrc. 20260416: infer latent shape using def
    ...
    def configure_optimizers(self):
        ...
        ## > modsrc: 20260416: Change optimizer Adam -> AdamW
        trainable_params = filter(lambda p: p.requires_grad, self.parameters())
        optimizer = th.optim.AdamW(trainable_params, lr=self.optimizer_params["learning_rate"], weight_decay=1e-3)
        lr_scheduler = th.optim.lr_scheduler.CosineAnnealingLR(
            optimizer,
            T_max=self.optimizer_params["max_steps"],
            eta_min=self.optimizer_params["eta_min"],
