...
def calculate_event_gap(group):
    """
    Calculates the azimuthal gap of an earthquake for a group of stations.

    Parameters
    -----------
    group
        A DataFrame containing the columns 'source_latitude',
        'source_longitude', 'receiver_latitude', and 'receiver_longitude'
        for all stations that recorded one event.

    Returns
    --------
    max_gaps: int
        The maximum azimuthal gap in degrees. If fewer than two stations are
        available, the single computed azimuth is returned instead.

    References:
        - Shearer, P. M. (2009). Introduction to Seismology. Cambridge University Press.
    """
    event_lat = group["source_latitude"].iloc[0]
    event_lon = group["source_longitude"].iloc[0]

    # Compute azimuths from the hypocenter to each station.
    azimuths = []
    for _, row in group.iterrows():
        _, az, _ = gps2dist_azimuth(
            event_lat, event_lon, row["receiver_latitude"], row["receiver_longitude"]
        )
        azimuths.append(az)
...
def create_h5_file(file_path, df, dtfl):
    ...
    FDSN_SERVERS = [
        "IRIS", "NOA", "INGV", "RESIF", "GEOFON", "ORFEUS", "GEONET"
    ]
    ...
    for i, row in df.iterrows():
        ...
        mtpi_list.append(row["mtpi"])
        aspect_list.append(row["aspect"])
        elevation_list.append(row["elevation"])

        # azimuthal gap (precomputed per event in main(), not per trace)
        azimuthal_gap.append(row["azimuthal_gap"])
    ...
    aspect_arr = np.array(aspect_list)
    elevation_arr = np.array(elevation_list)
    ...
    with h5py.File(file_path, "w") as h5f:
        ...
        h5f.create_dataset("aspect", data=aspect_arr)
        h5f.create_dataset("elevation", data=elevation_arr)
...
def main():
    ...
    computing per-event azimuthal gaps, and creating an HDF5 file.
    """
    Path = os.path

    file_name = Path.expanduser(Path.normpath(input("Path of the HDF5 file: ")))
    csv_file = Path.expanduser(Path.normpath(input("Path of the CSV file: ")))
    output_path = Path.expanduser(Path.normpath(input("Desired path of the output file: ")))

    if not os.path.exists(output_path): os.makedirs(output_path)

    # Checkpoint: check for existing output version. Delete if exists
    output_file = os.path.join(output_path, "raw_waveforms.h5")
    try:
        if os.path.exists(output_file): 
            os.remove(output_file)
            print("Deleted a previous output file from previous run.")
    except Exception as e:
        print(f"Error in deleting previous output file: {e}")
    ...
    # Create a permanent event fingerprint so traces from the same event
    # can be grouped together for the azimuthal gap calculation.
    df["event_key"] = (
        df["source_origin_time"].astype(str)
        + "_"
        + df["source_latitude"].astype(str)
        + "_"
        + df["source_longitude"].astype(str)
    )

    # Thin DataFrame for RAM safety during groupby
    temp_df = df[
        ["event_key", "source_latitude", "source_longitude", 
         "receiver_latitude", "receiver_longitude"]].copy()

    # Compute the azimuthal gap once per event, then map it back onto every trace
    gap_map = temp_df.groupby("event_key").apply(calculate_event_gap, include_groups=False).to_dict()
    df["azimuthal_gap"] = df["event_key"].map(gap_map)

    del temp_df, gap_map
    gc.collect()

    # Create a new HDF5 file with selected events
    create_h5_file(output_file, df, dtfl)
